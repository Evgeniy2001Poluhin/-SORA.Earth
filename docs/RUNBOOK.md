...
> **Раздел 2 утрачен.** Текст «Эскалация через Timeweb VNC (Recovery
> Console)» выше висит без заголовка — это его тело; заголовок уничтожен в
> первом коммите `997de75` (7 мая). Судя по содержанию, раздел был о потере
> SSH-доступа (#296).
>
> Отдельно: весь этот runbook описывает инфраструктуру, которой нет, — см. #286.
...
> **Раздел 7 утрачен**, там же и тогда же. Соседи — восстановление доступа
> через Timeweb и работа с MLflow; чем был седьмой, из файла не следует (#296).
