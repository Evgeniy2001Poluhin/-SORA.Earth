...
Откат делается вперёд или из бэкапа:
...
# 2. Если схема должна вернуться — только восстановление из бэкапа,
#    снятого ДО апгрейда. См. docs/BACKUP_RESTORE.md.

# 3. Если четыре таблицы надо убрать осознанно:
#    удалить их явно, затем
docker compose -f docker-compose.prod.yml exec backend alembic stamp e3f8a7c15d92
```

**Снимать бэкап до апгрейда обязательно** — именно он делает второй вариант
возможным. Без него остаются только первый и третий.
