...
| 3 | Model storage | 3–8 weeks | #191 with #199 items 4/6. **Settled 2026-08-16:** `models/` **is** the seed directory; `runtime/staged/<run_id>/` and `runtime/active/` live on a separate named volume. `models/seed/` was measured and rejected — 37 hardcoded references across 18 files for a change of name, while the separation already comes from `runtime/`. | Fresh clone starts; retrain does not dirty Git; losing runtime falls back to seed | settled |
...
| EnvironmentalObservation schema | exists, five ingesters | `app/database.py` → `EnvironmentalObservation`; the count is `app/ingesters`, bound by `tests/test_the_ingester_count_is_counted.py` |
