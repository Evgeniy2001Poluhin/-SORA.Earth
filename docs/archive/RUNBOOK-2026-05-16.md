> # Исторический документ. Не выполнять.
>
> **Перестал быть верным не позднее 2026-08-16**, когда прежний сервер был
> удалён за неоплату. Здесь он лежит целиком и без правок: это запись того,
> как выглядела эксплуатация в мае 2026, а не инструкция.
>
> Проверено 2026-09-08 — из первой таблицы не отвечает ничего:
>
> | из runbook | сегодня |
> |---|---|
> | `https://sora-earth.ru` | **530** (Cloudflare: туннель есть, origin недоступен) |
> | `https://api.sora-earth.ru` | **530** |
> | `https://grafana.sora-earth.ru` | **530** |
> | `https://mlflow.sora-earth.ru` | тот же домен, тот же туннель |
> | `ssh.sora-earth.ru:2222`, `ssh sora-cf` | слушателей на 2222 — 0, `cloudflared` inactive |
> | `109.73.194.26` (Timeweb) | не хост этого проекта |
> | `/opt/sora_earth` | нет такого каталога |
>
> Живое сегодня: `https://sora-earth.online/health` → **200**, хост
> `77.110.118.93`, каталог `/opt/sora_earth_ai_platform`, Grafana и Prometheus
> только на `127.0.0.1` через SSH-туннель.
>
> **Где актуальные операции:** раздел «Production Server» в `CLAUDE.md` —
> он поддерживается и описывает нынешний хост. Развёртывание там ровно одно:
> `./scripts/deploy_production.sh`. Раздел 1 ниже предлагает
> `docker compose up -d`; после инцидента 2026-08-09 (#129) это прямо
> запрещено — ручной `up` поднял второй контейнер приложения, тот занял адрес,
> nginx остался на старом, и публичный сайт отдавал 502 четыре с половиной
> минуты при всех healthy.
>
> Переписать под текущий хост — отдельная работа (#286, вариант 1): Cloudflare
> Tunnel заменён прямым SSH, публичных поддоменов у Grafana и Prometheus
> больше нет, а какие процедуры сейчас реальны, знает владелец, а не этот
> файл.
