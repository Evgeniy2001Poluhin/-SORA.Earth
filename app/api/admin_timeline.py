...
def admin_timeline(
    ...
    for r in retrains:
        events.append({"type": "retrain", "timestamp": r.started_at.isoformat() if r.started_at else None, "status": r.status, "trigger_source": r.trigger_source, "duration_sec": r.duration_sec, "model_version": r.model_version, "message": r.message, "error_message": r.error_message})
