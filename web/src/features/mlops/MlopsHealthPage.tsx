...
export function MlopsHealthPage() {
  ...
  if (h.isError || st.isError || m.isError)
    return <div className="card-body"><p style={{ color: "var(--danger, #EF4444)" }}>Failed to load MLOps status</p></div>;
  ...
  const metricBars = [
    { name: "accuracy", value: metrics?.metrics?.accuracy ?? 0 },
    { name: "f1",       value: metrics?.metrics?.f1_score ?? 0 },
    { name: "roc_auc",  value: metrics?.metrics?.roc_auc ?? 0 },
    { name: "best_f1",  value: metrics?.metrics?.best_f1 ?? 0 },
  ];
    ...
    <div className="card-body" style={{ padding: 32 }}>
      ...
      <div className="kpi-grid">
        <div className="kpi">
          ...
          <div className="kpi-val" style={{ color: modelColor, fontSize: 18 }}>{(health?.model_status ?? "unknown").toUpperCase()}</div>
        </div>
        <div className="kpi">
          <div className="kpi-lbl">Drift</div>
          <div className="kpi-val" style={{ color: driftColor, fontSize: 18 }}>{(health?.drift_status ?? "unknown").replace("_", " ").toUpperCase()}</div>
        </div>
        <div className="kpi">
          <div className="kpi-lbl">Observations</div>
          <div className="kpi-val tabular">{health?.observations_tracked ?? 0}</div>
        </div>
        <div className="kpi">
          <div className="kpi-lbl">Threshold</div>
          <div className="kpi-val tabular">{(status?.current_threshold ?? 0).toFixed(2)}</div>
        </div>
      </div>
      ...
      <div className="mlops-section">
        ...
        <div style={{ height: 220, background: "var(--bg-2)", border: "1px solid var(--line)", borderRadius: 10, padding: 16 }}>
          <ResponsiveContainer width="100%" height={188}>
            <BarChart data={metricBars}>
              ...
              <Bar dataKey="value" fill="var(--planet)" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="mlops-section">
        <div className="eyebrow" style={{ marginBottom: 12 }}>Model artifacts · {metrics?.models_available?.length ?? 0}</div>
