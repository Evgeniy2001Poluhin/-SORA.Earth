...
const STATUS:any={fitted:true,n_samples:734,n_features:7,fitted_at:"2026-05-07T08:00:00Z",baseline_id:"bl-2026-05"};
...
export const driftBaselineApi={
  ...
  fit:(csv_path="data/projects.csv")=>isMock?delay({status:"fitted",n_samples:734} as any):api<DriftBaselineFitResponse>(`/mlops/drift/baseline/fit?csv_path=${encodeURIComponent(csv_path)}`,{method:"POST"}),
  ...
  simulate:(mode:"stable"|"drift"|"custom",n=50,shift?:number)=>{
    if(isMock)return delay({status:"simulated",mode,n,drift_detected:mode==="drift"} as any);
