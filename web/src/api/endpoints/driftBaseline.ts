 ...
 *
 *  Discriminated on `status`, because the verdict and the feature table only
 *  mean anything together with it. `drift_detected` is `null` whenever nothing
 *  was measured -- it used to be `false`, which reads as "looked and found
 *  none". `status` was previously typed as a bare `string` and never read by
 *  anyone, which is how the mock came to say "OK" while the server said "ok". */
type ModelDriftBase={window:number;observations:number;features:Record<string,KsFeatureStat>;reason_code:string|null};
export type ModelDriftKsResp =
  | (ModelDriftBase & {status:"ok"; drift_detected:boolean})
  | (ModelDriftBase & {status:"no_log"|"insufficient_data"; drift_detected:null});
