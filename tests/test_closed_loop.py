...
def _drift(detected: bool, features=None):
    ...
    return ModelDriftMeasured(
        ...
        observations=100, features=features or {}, reason_code=None,
    )
...
def test_auto_retrain_skips_when_no_drift():
    token = _admin_token()
    with patch("app.api.drift.compute_drift") as mocked_drift:
        mocked_drift.return_value = _drift(False)
